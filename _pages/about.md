...
.ria-hint{display:flex;align-items:center;gap:.45rem;font-size:.7rem;letter-spacing:.06em;
  color:var(--ria-mut);margin:.1rem 0 0;min-height:1.1rem;transition:opacity .45s ease}
.ria-hint{opacity:.75}
...
/* ---- rail arrows ---- */
.ria-nav{position:absolute;top:calc(2rem + clamp(15.5rem,76vw,20.5rem)/3);transform:translateY(-50%);
  ...
  cursor:pointer;padding:0;opacity:.94;
...
.ria-prev{left:-.1rem}
.ria-next{right:-.1rem}
.ria-nav:hover{border-color:var(--global-theme-color);color:var(--global-theme-color);opacity:1;transform:translateY(-50%) scale(1.07)}
.ria-nav:focus-visible{outline:2px solid var(--global-theme-color);outline-offset:2px}
.ria-nav:active{transform:translateY(-50%) scale(.96)}
...
@media (prefers-reduced-motion:reduce){
  .ria-nav{transition:none}
  .ria-nav:hover{transform:translateY(-50%)}
}
</style>
